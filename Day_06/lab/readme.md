...
If you finish lab early consider:
  ...
  - **SUPER BONUS** - Once you finish this whole lab, add a second model for comments, it should have the name of the person who wrote it, and some text for the comment (maybe time stamps?). This model should 'belong' the the post, the data should be related in some way. Do your own research on how to set up a `one-to-many` relationship (one post can have many comments, one comment only belongs to one post), in MongoDB. Use Mongo Documents, Google or [Matt's Notes](https://git.generalassemb.ly/Software-Engineering-Immersive-Remote/SEIR-Baby-Yoda/tree/b989a0bd45ae7cc7025494748a3a9277e707dbf1/unit_2/w06d03/instructor_notes) - note we will not have class time to teach a second, related model. 
...
#### Restful Routes
|#|Action|URL|HTTP Verb|liquid view filename|mongoose method|
|:---:|:---:|:---:|:---:|:---:|:---:|
|1| Index | /logs/ | GET | index.liquid | Log.find()|
|2| Show |||||
|3| New | /logs/new | GET | new.liquid | none |
...
1. create `index.liquid` and change your `res.send` to `res.render` this page
1. upgrade  your route and liquid to render all the logs in your database, just make an unordered list of the titles for now
...
1. upgrade your `index.liquid` so that each title links to its show page
1. Create a `show.liquid` and add HTML
 - show the title
 - show the entry
...
1. upgrade your `res.send` to a `res.render` of your `show.liquid`
1. don't forget to `git add` and `git commit` your work, give yourself an informative commit message so you can trace back your work, if you need to

### Delete Route
1. Fill out your Restful table
1. in your `index.liquid`, add a delete form
...
### Edit Route
1. Fill out your Restful table
1. in your `index.liquid`, add a link to your edit route
1. create your edit route in your `server.js`
1. create your `edit.liquid`
1. test it to make sure it works as expected (be sure to populate your form with your log's data)
1. don't forget to `git add` and `git commit` your work, give yourself an informative commit message so you can trace back your work, if you need to

### Put Route
1. Fill out your Restful table
1. upgrade your` edit.liquid` form to have the appropriate action and method
...
1. research liquid partials and implement them
